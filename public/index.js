...
$(document).ready(function () {
    ...
    //             marker.bindPopup(`${obj.name}<br>${address}`).openPopup()
    ...
    function renderDisplay(object) {
        $("#display").append(`<div>${objJ[0].name}</div>`)
        $("#display").append(`<div>${objJ[0].address}</div>`)
        $("#display").append(`<div>${objJ[0].phone}</div>`)
        $("#display").append(`<div>${objJ[0].hours}</div>`)
        $("#display").append(`<a href="/restaurant/${objJ[0].id}" id="bottom-of-display"><div>visit site</div></a>`)
    }
    renderDisplay(objJ[0])
})
